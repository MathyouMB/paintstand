...
RSpec.describe("Update Image") do
  ...
  describe Mutations::Images::UpdateImage do
    it "if conditions are valid, image is succesfully updates" do
      prepare_context({
        current_user: @user,
      })
      ...
      query = <<~GRAPHQL
        mutation updateImage($id: Int!, $title: String!) {
      ...
      expect(result["data"]["updateImage"]["id"].to_i).to(eq(image.id))
      expect(result["data"]["updateImage"]["title"]).to(eq(Image.last.title))
    end
